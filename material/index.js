...
var url = require('url');
var list = {username: "admin", pwd: "admin"};
var server = http.createServer(function(req,res){
    ...
    switch(ext){
        case ".css":
        case ".js":
        var reaurl = (instead == 'css')?'.'+req.url:("."+req.url).replace("/" + instead,'');
        fs.readFile(reaurl, 'utf-8',function (err, data) {
        ...
        break;
        default:
                if(req.method == 'GET' && (pathname.substr(-1) == 'g')){
                    var reaurl = (instead == 'images')?'.'+req.url:("."+req.url).replace("/" + instead,'');
                    fs.stat(reaurl,function(err,stats){
                        if(err || !stats.isFile()){
                            res.writeHead(404);
                            res.end('Not Found');
                            return;
                        }
                        serve(reaurl,mime.getType(ext.replace(".","")));
                    });
                }else if(req.method == 'GET' && req.url == '/list/'){
                    serve(path.join(__dirname,'/chapterList.html') ,'text/html');
                }else if(req.method == 'GET' && req.url == '/login/'){
                    serve(path.join(__dirname,'/login.html') ,'text/html');
                }else if(req.method == 'GET' && pathname == '/listmanager/'){
                    serve(path.join(__dirname,'/list.html') ,'text/html');
                }else if(req.method == 'GET' && req.url == '/addChapter/'){
                    serve(path.join(__dirname,'/addChapter.html') ,'text/html');
                }else if(req.method == 'GET' && pathname== '/detail'){
                    serve(path.join(__dirname,'/chapter.html') ,'text/html');
                }else if(req.method == 'GET' && pathname == '/getDetail'){
                    res.end(JSON.stringify(chapterList[url.parse(req.url,true).query.chapterId - 1]));
                }else if(req.method == 'GET' && pathname == '/login'){
                    var username =url.parse(req.url,true).query.username;
                    var pwd =url.parse(req.url,true).query.pwd;
                    if(username == list.username && pwd == list.pwd){
                        res.writeHead(302,{
                            'Location': 'http://localhost:8083/listmanager/'
                        })
                        res.end();
                    }
